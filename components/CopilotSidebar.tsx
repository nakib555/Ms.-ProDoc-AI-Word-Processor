...
export const CopilotSidebar: React.FC = () => {
  ...
  return (
    <div className="w-[350px] bg-white dark:bg-[#1e293b] border-l border-slate-200 dark:border-slate-800 flex flex-col shadow-xl z-20 transition-all duration-300">
        ...
        {/* Chat Area */}
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-600 bg-[#f9fafb] dark:bg-[#0f172a]">
            {messages.map((msg) => (
                <div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                    <div 
                        className={`max-w-[90%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm ${
                            ...
                            : 'bg-white dark:bg-[#1e293b] text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700/50 rounded-bl-none'
                        }`}
                    >
        ...
        {/* Input Area */}
        <div className="p-4 bg-white dark:bg-[#1e293b] border-t border-slate-200 dark:border-slate-700/50">
            ...
            <div className="mt-2 flex gap-2 overflow-x-auto no-scrollbar pb-1">
                {['Summarize document', 'Draft an intro', 'Make it professional'].map(suggestion => (
                    <button 
                        ...
                        className="whitespace-nowrap px-3 py-1.5 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-700 rounded-full text-xs text-slate-500 dark:text-slate-400 hover:border-purple-400 dark:hover:border-purple-500 hover:text-purple-600 dark:hover:text-purple-400 transition-colors"
                    >
                        {suggestion}
                    </button>
                ))}
