      ...
      return (
        ...
        >
            <button
                ...
                className={`flex items-center w-full px-2 py-1 text-left hover:bg-slate-100 rounded-sm group transition-colors ${activeMenu === id ? 'bg-slate-100 text-blue-700' : ''}`}
            >
   ...
   return (
      ...
      >
          <button
              onClick={(e) => { e.stopPropagation(); !disabled && toggleMenu(id); }}
              onMouseDown={(e) => e.preventDefault()}
              className={`flex flex-col items-center justify-center px-1 py-1 min-w-[60px] md:min-w-[68px] h-full rounded-lg transition-all duration-200 group relative text-slate-600 hover:text-blue-700 hover:bg-slate-50 ${activeMenu === id ? 'bg-slate-100 text-blue-700 shadow-inner ring-1 ring-slate-200' : ''}`}
          >
              <div className="p-1.5 rounded-md group-hover:bg-white group-hover:shadow-sm transition-all mb-0.5">
              ...
              <div className="flex items-center justify-center w-full px-0.5">
                  <span className="text-[10px] sm:text-[11px] font-medium leading-tight text-center">{label}</span>
...
export const SmallRibbonButton: React.FC<{ icon: any, label: string, onClick: () => void, className?: string }> = React.memo(({ icon: Icon, label, onClick, className }) => (
  <button 
    onClick={onClick}
    onMouseDown={(e) => e.preventDefault()}
    className={`flex items-center w-full px-2 py-1 text-left hover:bg-slate-100 rounded-sm group transition-colors ${className || ''}`}
    title={label}
  >
